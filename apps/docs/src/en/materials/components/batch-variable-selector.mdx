...
- 🔍 **Auto Filtering**: Only displays array-type (`type: 'array'`) variables, based on [JSON Schema filtering](../../guide/variable/variable-consume#drilling-down-array-type-variables)
- 🔐 **Private Scope**: Provides isolated variable scope through [`PrivateScopeProvider`](../../guide/variable/concept#node-private-scope)
- 🎯 **Specialized Scenarios**: Designed for batch processing, loops, and other scenarios requiring array data sources, refer to [batch variable implementation ideas](../../guide/variable/cases/case-batch-variable)
...
The most common use case for BatchVariableSelector is in Loop nodes to select the array data source to iterate over:

```tsx pure title="loop-form-meta.tsx"
import { BatchVariableSelector, provideBatchInputEffect } from '@flowgram.ai/form-materials';

export const LoopFormRender = ({ form }) => {
  return (
    <>
      <FormHeader />
...
In nodes that need to process batch data, using BatchVariableSelector ensures users can only select array-type data sources:

```tsx pure
<Field<string[] | undefined> name="batchInput">
  {({ field }) => (
    <BatchVariableSelector
      value={field.value}
      onChange={(value) => field.onChange(value)}
      config={{ placeholder: 'Select batch data source' }}
    />
  )}
</Field>
```
